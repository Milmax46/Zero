...
const Monitor = (module.exports = {
  ...
  countConnection(ip, name = "") {
    ...
    if (count === 500) {
      this.adminlog(
        `[ResourceMonitor] IP ${ip} banned for cflooding (${count} times in ${Chat.toDurationString(
          duration
        )}${name ? ": " + name : ""})`
      );
      return true;
    ...
    if (count > 500) {
      if (count % 500 === 0) {
        ...
        if (c === 2 || c === 4 || c === 10 || c === 20 || c % 40 === 0) {
          this.adminlog(
            `[ResourceMonitor] IP ${ip} still cflooding (${count} times in ${Chat.toDurationString(
              duration
            )}${name ? ": " + name : ""})`
          );
        }
  ...
  countBattle(ip, name = "") {
    ...
    if (duration < 5 * 60 * 1000 && count % 30 === 0) {
      this.adminlog(
        `[ResourceMonitor] IP ${ip} has battled ${count} times in the last ${Chat.toDurationString(
          duration
        )}${name ? ": " + name : ""})`
      );
      return true;
  ...
  countConcurrentBattle(count, connection) {
    ...
    connection.popup(
      `Due to high load, you are limited to 5 games at the same time.`
    );
    return true;
  },
  /**
...
Monitor.cleanInterval = setInterval(
  () => Monitor.clean(),
  MONITOR_CLEAN_TIMEOUT
);
